...
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  if (input->type == kTfLiteInt8) {
    ...
    nmsis_nn_transpose_conv_params conv_params;
    ...
    const size_t buf_size = riscv_transpose_conv_s8_get_buffer_size(
...
TfLiteStatus EvalQuantizedPerChannel(TfLiteContext* context, TfLiteNode* node,
  ...
  ctx.size = 0;  // Note: ctx.size is currently not used in nmsis_nn.
  ctx.buf = context->GetScratchBuffer(context, data.scratch_buffer_index);

  nmsis_nn_context scratch_output_ctx;
  scratch_output_ctx.size =
      0;  // Note: ctx.size is currently not used in nmsis_nn.
