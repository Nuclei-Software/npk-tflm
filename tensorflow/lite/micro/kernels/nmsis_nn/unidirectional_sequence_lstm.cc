...
struct OpData {
  ...
  nmsis_nn_lstm_params params_nmsis_nn;
};
...
TfLiteStatus CalculateOpData(TfLiteContext* context, TfLiteNode* node,
  ...
  op_data->params_nmsis_nn.output_state_offset =
  ...
  op_data->params_nmsis_nn.i2f_effective_bias = input_to_forget_effective_bias;
  op_data->params_nmsis_nn.r2f_effective_bias =
      recurrent_to_forget_effective_bias;
  op_data->params_nmsis_nn.i2c_effective_bias = input_to_cell_effective_bias;
  op_data->params_nmsis_nn.r2c_effective_bias =
      recurrent_to_cell_effective_bias;
  op_data->params_nmsis_nn.i2o_effective_bias = input_to_output_effective_bias;
  op_data->params_nmsis_nn.r2o_effective_bias =
      recurrent_to_output_effective_bias;
  op_data->params_nmsis_nn.i2i_effective_bias = input_to_input_effective_bias;
  op_data->params_nmsis_nn.r2i_effective_bias =
  ...
  op_data->params_nmsis_nn.cell_state_shift = cell_scale;
  ...
  QuantizeMultiplier(
      ...
      &op_data->params_nmsis_nn.input_to_input_scaling.multiplier,
      &shift_output);
  op_data->params_nmsis_nn.input_to_input_scaling.shift =
      static_cast<int32_t>(shift_output);

  QuantizeMultiplier(
      static_cast<double>(effective_recurrent_to_input_scale),
      &op_data->params_nmsis_nn.recurrent_to_input_scaling.multiplier,
      &shift_output);
  op_data->params_nmsis_nn.recurrent_to_input_scaling.shift =
      static_cast<int32_t>(shift_output);
  QuantizeMultiplier(static_cast<double>(effective_cell_to_input_scale),
                     &op_data->params_nmsis_nn.cell_to_input_scaling.multiplier,
                     &shift_output);
  op_data->params_nmsis_nn.cell_to_input_scaling.shift =
      static_cast<int32_t>(shift_output);
  QuantizeMultiplier(
      static_cast<double>(effective_input_to_forget_scale),
      &op_data->params_nmsis_nn.input_to_forget_scaling.multiplier,
      &shift_output);
  op_data->params_nmsis_nn.input_to_forget_scaling.shift =
      static_cast<int32_t>(shift_output);
  QuantizeMultiplier(
      static_cast<double>(effective_recurrent_to_forget_scale),
      &op_data->params_nmsis_nn.recurrent_to_forget_scaling.multiplier,
      &shift_output);
  op_data->params_nmsis_nn.recurrent_to_forget_scaling.shift =
      static_cast<int32_t>(shift_output);
  QuantizeMultiplier(
      static_cast<double>(effective_cell_to_forget_scale),
      &op_data->params_nmsis_nn.cell_to_forget_scaling.multiplier,
      &shift_output);
  // ok
  op_data->params_nmsis_nn.cell_to_forget_scaling.shift =
      static_cast<int32_t>(shift_output);
  QuantizeMultiplier(static_cast<double>(effective_input_to_cell_scale),
                     &op_data->params_nmsis_nn.input_to_cell_scaling.multiplier,
                     &shift_output);
  op_data->params_nmsis_nn.input_to_cell_scaling.shift =
      static_cast<int32_t>(shift_output);
  QuantizeMultiplier(
      static_cast<double>(effective_recurrent_to_cell_scale),
      &op_data->params_nmsis_nn.recurrent_to_cell_scaling.multiplier,
      &shift_output);
  op_data->params_nmsis_nn.recurrent_to_cell_scaling.shift =
      static_cast<int32_t>(shift_output);
  QuantizeMultiplier(
      static_cast<double>(effective_input_to_output_scale),
      &op_data->params_nmsis_nn.input_to_output_scaling.multiplier,
      &shift_output);
  op_data->params_nmsis_nn.input_to_output_scaling.shift =
      static_cast<int32_t>(shift_output);
  QuantizeMultiplier(
      static_cast<double>(effective_recurrent_to_output_scale),
      &op_data->params_nmsis_nn.recurrent_to_output_scaling.multiplier,
      &shift_output);
  op_data->params_nmsis_nn.recurrent_to_output_scaling.shift =
      static_cast<int32_t>(shift_output);
  QuantizeMultiplier(
      static_cast<double>(effective_cell_to_output_scale),
      &op_data->params_nmsis_nn.cell_to_output_scaling.multiplier,
      &shift_output);
  op_data->params_nmsis_nn.cell_to_output_scaling.shift =
      static_cast<int32_t>(shift_output);

  op_data->params_nmsis_nn.projection_scaling.shift =
      static_cast<int32_t>(shift_output);

  QuantizeMultiplier(static_cast<double>(effective_hidden_scale),
                     &op_data->params_nmsis_nn.hidden_scaling.multiplier,
                     &shift_output);
  op_data->params_nmsis_nn.hidden_scaling.shift =
      static_cast<int32_t>(shift_output);

  op_data->params_nmsis_nn.hidden_offset = intermediate_zp[4];

  op_data->params_nmsis_nn.activation.min = std::numeric_limits<int16_t>::min();
  op_data->params_nmsis_nn.activation.max = std::numeric_limits<int16_t>::max();

  return kTfLiteOk;
}

template <typename CellType>
TfLiteStatus NMSIS_NN_EvalInteger8x8_16Lstm(
  ...
  nmsis_nn_lstm_params cmsis_lstm_params = op_data.params_nmsis_nn;
    ...
    case kTfLiteInt8: {
      switch (weight_type) {
        case kTfLiteInt8: {
          ...
          return NMSIS_NN_EvalInteger8x8_16Lstm<int16_t>(
...
TfLiteStatus UnidirectionalSequenceLstmEvalInt8(TfLiteContext* context,
  ...
  if (activation_type == kTfLiteInt8) {
    ...
    return NMSIS_NN_EvalInteger8x8_16Lstm<int16_t>(op_data, kernel_content,
                                                   buffers);
  } else {
    MicroPrintf("Input type %s (%d) not supported.",
                TfLiteTypeGetName(activation_type), activation_type);
