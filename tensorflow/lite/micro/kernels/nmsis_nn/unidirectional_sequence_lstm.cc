...
struct OpData {
  ...
  nmsis_nn_lstm_params params_nmsis_nn;  // Used for  NMSIS-NN implementation
};

LSTMBuffers<int16_t> NMSIS_NN_CreateLSTMBuffers(TfLiteContext* context,
...
void NMSIS_NN_VectorSum(int32_t* kernel_sum, const int32_t size1,
                        const int32_t size2, const int8_t* weights,
                        const int32_t offset, const int32_t* biases) {
  riscv_vector_sum_s8(kernel_sum, size1, size2, weights, offset, 0, biases);
}

void NMSIS_NN_VectorSum(int64_t* kernel_sum, const int32_t size1,
                        const int32_t size2, const int8_t* weights,
                        const int32_t offset, const int64_t* biases) {
  riscv_vector_sum_s8_s64(kernel_sum, size1, size2, weights, offset, biases);
}

template <typename BiasType>
TfLiteStatus NMSIS_NN_PortOpData(TfLiteContext* context, OpDataLSTM* params_ref,
  ...
  nmsis_nn_lstm_gate gate_forget{
  ...
  auto cell_gate_nonlinear_type =
      (params_ref->cell_gate_nonlinear_type == kTfLiteActTanh) ? RISCV_TANH
                                                               : RISCV_SIGMOID;
  // Create cell gate parameters
  nmsis_nn_lstm_gate gate_cell{
  ...
  nmsis_nn_lstm_gate gate_output{
...
TfLiteStatus NMSIS_NN_EvalInteger8x8_16Lstm(
  ...
  nmsis_nn_lstm_context nmsis_buffers;
  nmsis_buffers.temp1 = reinterpret_cast<int16_t*>(buffers.buffer0);
  nmsis_buffers.temp2 = reinterpret_cast<int16_t*>(buffers.buffer1);
  nmsis_buffers.cell_state = reinterpret_cast<int16_t*>(buffers.buffer2);

  riscv_lstm_unidirectional_s8(input, output, &op_data.params_nmsis_nn,
                             &nmsis_buffers);

  return kTfLiteOk;
}

TfLiteStatus NMSIS_NN_EvalInteger16x8_16Lstm(
    const OpData& op_data, const LSTMKernelContents& kernel_content,
    const LSTMBuffers<int16_t>& buffers) {
  TFLITE_DCHECK(
      kernel_content.GetInternalTensor(tflite::kLstmInputTensor)->dims->size >=
  ...
  riscv_lstm_unidirectional_s16(input, output, &op_data.params_nmsis_nn,
                              &nmsis_buffers);
...
TfLiteStatus UnidirectionalSequenceLstmPrepare(TfLiteContext* context,
  ...
  if (activation_type == kTfLiteInt8 && cell_state_type == kTfLiteInt16) {
    ...
    number_of_buffers = 3;
    NMSIS_NN_PortOpData<int32_t>(context, op_data_lstm, kernel_content,
                                 &op_data->params_nmsis_nn);
  } else if (activation_type == kTfLiteInt16 &&
             cell_state_type == kTfLiteInt16) {
    auto kernel_content = CreateLSTMKernelContent(context, node);
    number_of_buffers = 3;
    NMSIS_NN_PortOpData<int64_t>(context, op_data_lstm, kernel_content,
                                 &op_data->params_nmsis_nn);
  } else {
    number_of_buffers = 4;
  }
    ...
    case kTfLiteInt8: {
      switch (weight_type) {
        case kTfLiteInt8: {
          ...
          LSTMBuffers<int16_t> buffers =
              NMSIS_NN_CreateLSTMBuffers(context, op_data_lstm.buffer_indices);
          NMSIS_NN_EvalInteger8x8_16Lstm(op_data, kernel_content, buffers);
          break;
        }
        default: {
    ...
    case kTfLiteInt16: {
      switch (weight_type) {
        case kTfLiteInt8: {
          ...
          LSTMBuffers<int16_t> buffers =
              NMSIS_NN_CreateLSTMBuffers(context, op_data_lstm.buffer_indices);
          NMSIS_NN_EvalInteger16x8_16Lstm(op_data, kernel_content, buffers);
          break;
        }
        default: {
          MicroPrintf("Filter type %s (%d) not supported.",
...
TfLiteStatus UnidirectionalSequenceLstmEvalInt8(TfLiteContext* context,
  ...
  if (activation_type == kTfLiteInt8) {
    ...
    return NMSIS_NN_EvalInteger8x8_16Lstm(op_data, kernel_content, buffers);
  } else {
    MicroPrintf("Input type %s (%d) not supported.",
                TfLiteTypeGetName(activation_type), activation_type);
    return kTfLiteError;
...
TfLiteStatus UnidirectionalSequenceLstmEvalInt16(TfLiteContext* context,
  ...
  if (activation_type == kTfLiteInt16) {
    ...
    return NMSIS_NN_EvalInteger16x8_16Lstm(op_data, kernel_content, buffers);
  } else {
    MicroPrintf("Input type %s (%d) not supported.",
                TfLiteTypeGetName(activation_type), activation_type);
    return kTfLiteError;
