...
TfLiteStatus PadEvalInt8(TfLiteContext* context, TfLiteNode* node) {
  ...
  const nmsis_nn_dims input_size = {d.Dims(0), d.Dims(1), d.Dims(2), d.Dims(3)};

  const PadParams p = data->params;
  const nmsis_nn_dims pre_pad = {p.left_padding[0], p.left_padding[1],
                                 p.left_padding[2], p.left_padding[3]};
  const nmsis_nn_dims post_pad = {p.right_padding[0], p.right_padding[1],
                                  p.right_padding[2], p.right_padding[3]};

  riscv_pad_s8(input_ptr, output_ptr, pad_value, &input_size, &pre_pad,
             &post_pad);
