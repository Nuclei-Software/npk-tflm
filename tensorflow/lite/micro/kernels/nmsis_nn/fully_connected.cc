...
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  if (input->type == kTfLiteInt16) {
    ...
    if (is_conv_1x1_possible) {
      ...
      buf_size = riscv_convolve_1x1_s8_fast_get_buffer_size(&input_dims);
    } else if (input->type == kTfLiteInt8) {
      buf_size = riscv_fully_connected_s8_get_buffer_size(&filter_dims);
...
void PopulateCommonParams(TfLiteContext* context,
                          nmsis_nn_per_tensor_quant_params* const quant_params,
                          nmsis_nn_dims* const input_dims,
                          nmsis_nn_dims* const filter_dims,
                          nmsis_nn_dims* const bias_dims,
                          nmsis_nn_dims* const output_dims,
                          nmsis_nn_context* const ctx, const OpData& data) {
...
TfLiteStatus EvalQuantizedInt8(TfLiteContext* context, TfLiteNode* node,
  ...
  if (output_dim_count > 2 && data.accum_depth % 4 == 0) {
    ...
    nmsis_nn_per_channel_quant_params per_channel_quant_params;
    if (data.reference_op_data.is_per_channel) {
    ...
    nmsis_nn_quant_params quant_params;
