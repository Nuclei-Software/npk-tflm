...
struct OpData {
  ...
  nmsis_nn_dims output_shape;

  int buffer_idx;
};

nmsis_nn_dims FillVariableShape(int32_t rank, int32_t* tensor_dims) {
  if (rank == 4) {
...
inline TfLiteStatus PopulateEvalData(
  ...
  *updated_lhs_input = params->adj_x
  ...
  // ReferenceOps and NMSIS-NN have different requirements for when the
...
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  if (lhs_input->type != kTfLiteFloat32 && rhs_input->type != kTfLiteFloat32) {
    ...
    if (lhs_input->type == kTfLiteInt8) {
      ...
      data->buffer_idx = -1;
      buf_size = riscv_fully_connected_s8_get_buffer_size(&data->output_shape);
    } else {
      data->reference_op_data.quantization->output_activation_min =
...
TfLiteStatus EvalInt8(TfLiteContext* context, TfLiteNode* node) {
  ...
  nmsis_nn_bmm_params bmm_params = {
      params->adj_x,
      params->adj_y,
      fc_params,
  };
...
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
  ...
  switch (original_lhs_input->type) {
    ...
    default: {
      MicroPrintf("NMSIS-NN Batch Matmul: Type %s (%d) not supported.",
